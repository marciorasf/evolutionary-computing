...
class RastriginStrategy(ProblemaStrategyAbstract):
    ...
    def gera_z(self, X, Y):
        Z = 20
        Z += X**2 - 10*np.cos(np.pi*2*X)
        Z += Y**2 - 10*np.cos(np.pi*2*Y)
        return Z
...
class SchwefelStrategy(ProblemaStrategyAbstract):
    ...
    def gera_z(self, X, Y):
        Z = 418.9829*2
        Z -= X*np.sin(np.sqrt(abs(X)))
        Z -= Y*np.sin(np.sqrt(abs(Y)))
        return Z
...
class Populacao:
    ...
    def get_coordenadas(self):
        ...
        y = []
        for individuo in self.individuos:
            x.append(individuo.variaveis[0])
            y.append(individuo.variaveis[1])
        return x,y
...
class Problema:
    ...
    def gera_z(self, X, Y):
        return(self.problema_strategy.gera_z(X,Y))
...
def run_DE():
    ...
    n_variaveis = 2
    tam_pop = 50
    max_iteracoes = 100*n_variaveis
    lim_inferior, lim_superior = problema.inicializa_limites(n_variaveis)
    precisao = 1e-12
    fator_escala = 0.5
    prob_mutante = 0.5

    X = np.linspace(lim_inferior[0], lim_superior[0], 1000)
    Y = np.linspace(lim_inferior[1], lim_superior[1], 1000)
    X, Y = np.meshgrid(X, Y)
    Z = problema.gera_z(X,Y)
    plt.ion()
    fig = plt.figure()
    ...
    x,y = populacao.get_coordenadas()
    linel, = points.plot(x,y, marker='o',color='red', linestyle='None' )
    fig.canvas.draw()
